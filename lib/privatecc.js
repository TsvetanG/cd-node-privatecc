...
var Chaincode = class {
	...
	async push(stub, args) {
		...
		if (args.length != 2) {
			return shim.error('Expecting client ID and private data as parameters');
		}
		...
		logger.info("Private for client : " + clientId + " data : " + args[1]);
		
		await stub.putPrivateData(collection, args[0], Buffer.from(args[1]));
		logger.info("Private stored");
	...
	async query(stub, args) {
		...
		if (args.length != 1) {
			return shim.error('Expecting client ID as parameter');
		}
		...
		let collection = "coll";
		let prvData = await stub.getPrivateData(collection, clientId);
		logger.info("Private fetched");
    	return shim.success(prvData);
	}
